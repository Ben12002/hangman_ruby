...
class Game
  ...
  def modify(word = "", lives = 5, curr_guess = "", curr_state = [], wrong_chars = [])
    @word = word
    @lives = lives
    @curr_guess = curr_guess # e.g "a"
    @curr_state = curr_state # e.g "__a__e__"
  ...
  def from_yaml(fname)
    ...
    modify(data[:word],data[:lives],data[:curr_guess],data[:curr_state], data[:wrong_chars])
  end
